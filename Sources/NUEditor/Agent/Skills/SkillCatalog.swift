...
/// Fetches the community skill catalog from the nueditor-skills repo (raw GitHub CDN)
@Observable
@MainActor
final class SkillCatalog {
    static let shared = SkillCatalog()

    /// Catalog source. Override with the NUEDITOR_SKILLS_BASE env var to test against a
    /// local clone, e.g. file:///path/to/nueditor-skills.
    static var base: String {
        ProcessInfo.processInfo.environment["NUEDITOR_SKILLS_BASE"]
            ?? "https://raw.githubusercontent.com/karthikra/nueditor-skills/main"
    }
