...
/// Reads skills from `~/.nueditor/skills/` — the single source of truth.
@Observable
@MainActor
final class SkillStore {
    ...
    nonisolated static var directory: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".nueditor/skills", isDirectory: true)
    }
    ...
    /// Resolves `~/.nueditor/skills/<id>/` only when `id` is a single safe path component.
    ...
    /// Copies under a `nueditor-` prefix so we only overwrite our own prior copy
    @discardableResult
    func copy(_ skill: Skill, to agent: SkillExternalAgent) -> URL? {
        let source = skill.path.deletingLastPathComponent()
        let dest = agent.skillsDirectory.appendingPathComponent("nueditor-\(skill.id)", isDirectory: true)
        let fm = FileManager.default
        do {
