...
enum ToolDefinitions {
    static let all: [AgentTool] = [
        ...
        AgentTool(
            name: .exportProject,
            description: "Queues an export from the current project using the same modes as the Export dialog. mode defaults to video. video renders H.264, H.265, or ProRes; xml writes XMEML timeline XML; fcpxml writes FCPXML; nueditor writes a self-contained .nueditor project package. For timeline interchange, pick the format by the target editor: Premiere Pro -> xml; DaVinci Resolve or Final Cut Pro -> fcpxml (fcpxml also carries text, transforms, crop, opacity, and keyframes that xml cannot). Video exports render edge softness and edge rounding, NUEditor project exports preserve them, and xml/fcpxml interchange omits them. Omit outputPath to write a unique file to ~/Downloads. Existing direct outputPath files are overwritten by default to match the UI save flow; pass overwrite=false to refuse. Every mode returns status=started or status=queued with a jobId and destination path. Use manage_exports to check progress, warnings/results, or cancel by jobId; agent exports post a system notification on completion or failure.",
            inputSchema: objectSchema(
                properties: [
                    "mode": ["type": "string", "enum": ["video", "xml", "fcpxml", "nueditor"], "description": "Optional. Default video. Use xml for Premiere Pro, fcpxml for DaVinci Resolve or Final Cut Pro."],
                    ...
                    "timelineId": ["type": "string", "description": "Optional. Timeline to export (from get_timeline's timelines list). Defaults to the active timeline. Not valid for nueditor mode, which packages every timeline."],
                ]
            )
        ),
        AgentTool(
                ...
                properties: [
                    "source": [
                        ...
                        "properties": [
                            ...
                            "path": ["type": "string", "description": "Absolute local file or directory path, readable by the NUEditor process. Files are referenced in place and must remain available. A directory is imported recursively and its folder structure is replicated as media folders."],
    ...
    static let manageProject = AgentTool(
        name: .manageProject,
        description: "List, open, create, or close NUEditor projects for this MCP session. Set `action` to: `list` for known projects plus session-active and visible state; `open` with a name, id from list, or .nueditor path; `create` with an optional name and initial fps/aspectRatio/quality; or `close` to save and close the session project, optionally targeting another open project by name/id/path. Opening or creating changes only this session's target. Closing always completes a final save first. This tool never deletes projects or files.",
        inputSchema: objectSchema(
            properties: [
                ...
                "path": ["type": "string", "description": "Filesystem path to a .nueditor package. Used by open or close."],
