...
private struct ExportAnalyticsRun {
    ...
    init(nueditorContext context: ExportAnalyticsContext) {
        self.basePayload = [
            "source": context.source,
            "project_id": context.projectId ?? "unknown",
            "mode": "nueditor",
...
final class ExportService {
    ...
    var lastNUEditorReport: NUEditorProjectExporter.Report?
    ...
    /// Writes a self-contained `.nueditor` bundle (all media collected internally).
    @discardableResult
    func exportNUEditorProject(
        ...
        outputURL: URL,
        analyticsContext: ExportAnalyticsContext = .init()
    ) async -> NUEditorProjectExporter.Report? {
        reset()
        defer { activeCancellation = nil }
        let analytics = ExportAnalyticsRun(nueditorContext: analyticsContext)
        ...
        do {
            ...
            setPhase(.exporting)
            Log.export.notice(
                "nueditor export start url=\(outputURL.lastPathComponent)",
                telemetry: "NUEditor project export started",
                data: [
            ...
            let worker = Task.detached(priority: .userInitiated) {
                try NUEditorProjectExporter.export(
            ...
            lastNUEditorReport = report
            didCommitOutput = true
            setProgress(1)
            Log.export.notice(
                "nueditor export ok collected=\(report.collected.count) missing=\(report.missing.count)",
                telemetry: "NUEditor project export finished",
            ...
            if Self.isCancellation(error) {
                wasCancelled = true
                Log.export.notice("nueditor export cancelled", telemetry: "Export cancelled")
            } else {
                self.error = Log.detail(error)
                Log.export.error(
                    "nueditor export failed: \(Log.detail(error))",
                    telemetry: "NUEditor project export failed",
