...
enum ExportDestination: String, CaseIterable, Identifiable {
    ...
    case nueditorProject = "NUEditor Project"
...
struct ExportView: View {
    ...
    @State private var nueditorSummary: (collect: Int, missing: Int, bytes: Int64) = (0, 0, 0)
    ...
    var body: some View {
        ...
        .task {
            ...
            let summary = await Task.detached(priority: .utility) {
                Self.computeNUEditorSummary(entries: entries, projectURL: projectURL)
            }.value
            guard !Task.isCancelled else { return }
            nueditorSummary = summary
        }
    }
        ...
        VStack(spacing: AppTheme.Spacing.zero) {
            VStack(alignment: .leading, spacing: AppTheme.Spacing.zero) {
                VStack(alignment: .leading, spacing: AppTheme.Spacing.zero) {
                    ...
                    if editor.timelines.count > 1, destination != .nueditorProject {
                    ...
                    case .timeline:
                        timelineSettings
                    case .nueditorProject:
                        nueditorProjectSettings
                    }
                }
    ...
    private var nueditorProjectSettings: some View {
        VStack(alignment: .leading, spacing: AppTheme.Spacing.xs) {
            Text("Saves a copy of this project with all media bundled inside, so it opens on any machine.")
                .font(.system(size: AppTheme.FontSize.sm))
                .foregroundStyle(AppTheme.Text.secondaryColor)

            if nueditorSummary.missing > 0 {
                Text("\(nueditorSummary.missing) media file\(nueditorSummary.missing == 1 ? "" : "s") missing - they'll be skipped.")
                    .font(.system(size: AppTheme.FontSize.xs))
                    .foregroundStyle(AppTheme.Status.errorColor)
            }
        }
        ...
        return HStack(spacing: AppTheme.Spacing.lg) {
            ...
            case .nueditorProject:
                HStack(spacing: AppTheme.Spacing.xs) {
                    ...
                    Text("~\(ByteCountFormatter.string(fromByteCount: nueditorSummary.bytes, countStyle: .file))")
                }
    ...
    private var exportFormat: ExportFormat {
        ...
        case .nueditorProject: .xml   // NUEditor Project has its own path; never rendered.
        case .video: codec.exportFormat
        }
    }

    /// Quick estimate for exporting a NUEditor Project
    private nonisolated static func computeNUEditorSummary(
    ...
    private func startExport() {
        if destination == .nueditorProject { startNUEditorExport(); return }
        submissionError = nil
    ...
    private func startNUEditorExport() {
        submissionError = nil
        let panel = NSSavePanel()
        ...
        panel.begin { response in
            guard response == .OK, let url = panel.url else { return }
            do {
                try exportQueue.enqueueNUEditorProject(
