...
/// Writes a self-contained `.nueditor` package: every resolvable media reference is copied
/// into the new bundle's `media/` directory and rewritten to a project-relative source
enum NUEditorProjectExporter {
    ...
    static func export(
        ...
        let staging = parent.appendingPathComponent(".nueditor-export-\(UUID().uuidString).partial", isDirectory: true)
