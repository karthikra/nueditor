...
struct MCPInstructionsPane: View {
    ...
    private var claudeCodeCommand: String {
        "claude mcp add --transport http nueditor \(mcpEndpoint)"
    }

    private var codexCommand: String {
        "codex mcp add nueditor --url \(mcpEndpoint)"
    }
    ...
    private var cursorDeepLink: URL? {
        ...
        return URL(string: "cursor://anysphere.cursor-deeplink/mcp/install?name=nueditor&config=\(encoded)")
    }

    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: AppTheme.Spacing.xxl) {
                Text("Connect an external agent to inspect and edit the open NUEditor project.")
    ...
    private var cursorSection: some View {
        agentSection(
            .cursor,
            name: "Cursor",
            description: "Install the NUEditor MCP server in Cursor.",
    ...
    private var claudeDesktopSection: some View {
        agentSection(
            .claude,
            name: "Claude Desktop",
            description: "Install the bundled NUEditor connector.",
    ...
    private func openClaudeDesktopBundle() {
        guard let bundleURL = claudeDesktopBundleURL else {
            claudeInstallError = "The NUEditor connector could not be found. Reinstall NUEditor, then try again."
            return
        }
        ...
        NSWorkspace.shared.open(
            ...
            Task { @MainActor in
                claudeInstallError = "Claude Desktop could not open the NUEditor connector. Update Claude Desktop, then try again."
            }
        }
    }

    private var claudeDesktopBundleURL: URL? {
        BundledResource.url("nueditor.mcpb")
    }
}
