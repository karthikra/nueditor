...
@Suite("NUEditor project export (collect media)")
struct NUEditorProjectExportTests {
    ...
    private func makeFixture() throws -> (root: URL, source: URL, dest: URL, externalContents: String) {
        ...
        let source = root.appendingPathComponent("Source.nueditor", isDirectory: true)
        ...
        return (root, source, root.appendingPathComponent("Packaged.nueditor", isDirectory: true), externalContents)
    }
    ...
    @Test func collectsExternalAndInternalRewritesSourcesReportsMissing() throws {
        ...
        let report = try NUEditorProjectExporter.export(
