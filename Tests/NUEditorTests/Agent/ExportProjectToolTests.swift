...
struct ExportProjectToolTests {
    ...
    @Test func exportsXMLForANonActiveTimelineById() async throws {
        ...
        let nueditorExport = await h.runRaw("export_project", args: ["mode": "nueditor", "outputPath": "/tmp/x.nueditor", "timelineId": String(other.id.prefix(8))])
        #expect(nueditorExport.isError)
        #expect(ToolHarness.textOf(nueditorExport).contains("nueditor"))
    }
    ...
    @Test func exportsXML() async throws {
        ...
        let nueditorURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("export-tool-\(UUID().uuidString).nueditor")
        defer { try? FileManager.default.removeItem(at: nueditorURL) }
        let nueditorExport = try await h.runOK("export_project", args: [
            "mode": "nueditor", "outputPath": nueditorURL.path,
        ]) as? [String: Any]
        try await waitForJob(from: nueditorExport, in: h.exportQueue)
